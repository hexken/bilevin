...
from plots.utils import astar_group_key, levin_group_key, phs_group_key, PdfTemplate, get_dfs
...
def plot_single_ax(
    ...
    if color is not None:
        ax.plot(x[mask], aggr_y[mask], color=None)
    else:
...
def main():
    ...
    phs_runs = Path(f"../phs/runs/{exp_name}_phs/")

    figs_dir = Path(f"final_runs_{exp_name}")
    figs_dir.mkdir(exist_ok=True)

    levin_data = get_dfs(levin_runs,levin_group_key)
    astar_data = get_dfs(astar_runs,astar_group_key)
    phs_data = get_dfs(phs_runs,phs_group_key)
    ...
    phs_figs, phs_train_dfs, phs_valid_dfs = group_and_plot(
        phs_runs, phs_group_key, batch_size=batch_size
    )
    figs = levin_figs + astar_figs + phs_figs
    # figs = phs_figs
    ...
    # colors = ["b", "c", "g", "m"]
    labels = []
    for run_name, dfs in itertools.chain(
        astar_train_dfs, levin_train_dfs, phs_train_dfs
    ):
        # for run_name, dfs in phs_train_dfs:
        print(run_name)
        solveds = []
        cols = []
        curr_lens = len(dfs[0])
        ...
        # color = colors.pop()
        ax.plot(df.index.values, df["mean"], label=run_name, color=None)
        plt.fill_between(
