...
doms_grouped = {
    "tri": [[r"\wittri\ 4", "tri4"], [r"\wittri\ 5", "tri5"]],
    "col": [[r"\witcol\ 4", "col4"], [r"\witcol\ 5", "col5"]],
    "stp": [[r"\stp\ 4", "stp4"], [r"\stp\ 5", "stp5"]],
    "pancake": [[r"\pancake\ 10", "pancake10"], [r"\pancake\ 12", "pancake12"]],
}
doms = [
    (r"\wittri\ 4", "tri4"),
    (r"\wittri\ 5", "tri5"),
...
def create_bi_table(
    ...
    for ldom, pdom in doms:
            ...
            if agent is not None:
                ...
                for _, prow, _ in data_rows:
                    if prow in dom_data[agent]:
                        ...
                        agents_data[at][agent][prow] = None
        ...
        for _, prow, cmp in data_rows:
            ...
            for at in agents_data:
                ...
                for agent in agents_data[at]:
                    if agents_data[at][agent][prow] is None:
                        continue
        ...
        for i, (lrow, prow, _) in enumerate(data_rows):
            ...
            if i == 0:
                if show_solved:
                    first_col = f"\\multirow{{2}}{{*}}{{\parbox{{1.25cm}}{{\\raggedright{ldom} ({count})}}}} & {lrow}"
                else:
...
def create_domain_tables(indir, data: dict[str, pd.DataFrame], data_rows, n_decimals=1, show_solved=True
):
    round_fmt = f".{n_decimals}f"

    for domname, doms in doms_grouped.items():
        latex_table = [
            r"\begin{table}[htbp]",
            r"\footnotesize",
            r"\centering",
        ...
        (indir / f"{domname}_table.tex").open("w").write(s)
...
if __name__ == "__main__":
    ...
    # s = create_common_table( solved_stats, all_data_rows, n_decimals=1, show_solved=False)
    # (indir / "all_table.tex").open("w").write(s)
    create_domain_tables(indir/ "doms_solved", solved_stats, all_data_rows, n_decimals=1, show_solved=False)
    create_domain_tables(indir / "doms_common", common_stats, common_data_rows, n_decimals=1)
