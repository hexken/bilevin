...
#SBATCH --array=1-2
#SBATCH --exclusive
#SBATCH --output=/scratch/tjhia/bilevin/slurm_outputs/pancake64_levin/%j.out
...
python src/main.py \
    ...
    --agent $agent \
    --seed $seed \
    --runsdir-path runs/pancake64_levin \
    --exp-name $expname \
    --problems-path problems/pancake64/50000-train.pkl \
    --valid-path problems/pancake64/1000-valid.pkl \
    --world-size 40 \
    --mode train \
    --loss-fn $loss \
    --grad-steps 10 \
